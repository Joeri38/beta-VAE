...
# model_v2 adds wandb to log metrics

class Network(object):

    def __init__(self, input_size, latent_size, input2_size, output_size, seed, activation=tf.nn.elu,
                 encoder_num_units=[100, 100], decoder_num_units=[100, 100], name='Unnamed',
                 tot_epochs=0, load_file=None, wandblog=False):
        """
        Parameters:
    ...
    def train(self, epoch_num, batch_size, learning_rate, training_data, validation_data,
        ...
        with self.graph.as_default():
            ...
            for epoch_iter in tqdm_notebook(range(epoch_num)):
                self.tot_epochs += 1
                if self.tot_epochs % 10 == 0:
                    print(f"Working on epoch {self.tot_epochs}...")
                ...
                for step, data_dict in enumerate(self.gen_batch(training_data, batch_size)):
                    ...
                    data_dict.update(parameter_dict)
                    feed_dict = data_dict

                    self.session.run(self.training_op, feed_dict=feed_dict)
                if self.tot_epochs % 10 == 0:
                    print(f"Finished epoch {self.tot_epochs}!\n")

                kl = self.run(training_data, self.kl_loss)
                recon = self.run(training_data, self.recon_loss)
                cost = recon + 0.001*kl
                mae_test = self.run(validation_data, self.mae_test)

                if self.wandblog:
                    wandb.log({'total_loss_train': cost,
                               'recon_loss_train': recon,
                               'kl_divergence_train': kl,
                               'mae_test': mae_test,
            ...
            with tf.compat.v1.variable_scope('encoder'):
                ...
                for k in range(len(enc_in_dims)):
                    with tf.compat.v1.variable_scope('{}th_enc_layer'.format(k)):
                        ...
                        b = tf.compat.v1.get_variable('b', [enc_out_dims[k]],
                                            initializer=tf.compat.v1.initializers.constant(0.))
                        squash = ((k + 1) != len(enc_in_dims))  # don't squash latent layer
                        temp_layer = forwardprop(temp_layer, w, b, act_fun=self.activation,
                                                 name='enc_layer_{}'.format(k), squash=squash)

            with tf.compat.v1.variable_scope('latent_layer'):
            ...
            with tf.compat.v1.variable_scope('decoder'):
                ...
                for k in range(len(dec_in_dims)):
                    with tf.compat.v1.variable_scope('{}th_dec_layer'.format(k)):
                        w = tf.compat.v1.get_variable('w', [dec_in_dims[k], dec_out_dims[k]],
                                            initializer=tf.compat.v1.initializers.random_normal(stddev=2. / np.sqrt(dec_in_dims[k] + dec_out_dims[k])))
                        b = tf.compat.v1.get_variable('b', [dec_out_dims[k]],
                                            initializer=tf.compat.v1.initializers.constant(0.))
                        squash = ((k + 1) != len(dec_in_dims))  # don't squash latent layer
                        temp_layer = forwardprop(temp_layer, w, b, act_fun=self.activation,
                                                 name='dec_layer_{}'.format(k), squash=squash)

                self.output = temp_layer

            with tf.compat.v1.name_scope('recon_loss'):
                self.recon_loss = tf.reduce_mean(tf.reduce_mean(tf.math.squared_difference(self.labels, self.output),
                                                 axis=1))

            with tf.compat.v1.name_scope('mae_test'):
                self.mae_test = tf.reduce_mean(tf.reduce_mean(tf.math.abs(tf.math.subtract(self.labels, self.output)),
                                               axis=1))
    ...
    def gen_batch(self, data, batch_size, shuffle=True, random_epsilon=True):
        ...
        """
        epoch_size = int(len(data[0]) / batch_size)
        if shuffle:
...
def forwardprop(x, w, b, act_fun, squash=True, name=''):
    """
    Forward-propagation.
    """
    if name != '':
...
def kl_divergence(means, log_sigma, dim, target_sigma=0.1):
    ...
    return 1 / 2. * tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=1 / target_sigma**2 * means**2 +
