...
def oscillator_data(N, t_sample=np.linspace(0, 5, 50), A_interval=[1, 1], delta_interval=[0, 0],
    ...
    t_full = np.linspace(0, 10, 50)
    t_meas = np.array([t_full for i in range(N)])

    # x_in is [0,5] time series, x_out is evaluation in [0, 10] time series t_meas
    x_out = []
